...
def test_auto_place_reverts_cmaes_if_displacement_exceeds_threshold(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(placement_module, "_refine_with_cmaes", fake_refine)
    monkeypatch.setattr(placement_module, "_resolve_remaining_conflicts", fake_resolve)
    # Le contrôle « hors contour » (2026-07-31) ne fait que journaliser, mais on
    # le neutralise pour garder ce test muet : sur ce fixture le GA parque
    # réellement R1/R2/R3 hors de la carte 60×40 (mesuré ~147,95 en local),
    # soit exactement le défaut qu'il signale.
