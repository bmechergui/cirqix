...
class DockerBuildContextTests(unittest.TestCase):
    ...
    def test_kicad_docker_job_is_blocking(self) -> None:
        ...
        self.assertIn("submodules: false", job)
        self.assertIn("persist-credentials: false", job)
        self.assertNotIn("--recursive", job)
        self.assertIn("git submodule update --init --depth=1 --", job)
        self.assertIn("services/kicad/kicad-tools", job)
        self.assertIn("services/kicad/circuit_synth", job)
        self.assertIn("rm -f ~/.ssh/id_ed25519", job)
