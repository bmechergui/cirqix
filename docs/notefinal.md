...
### Pipeline complet (mis à jour 2026-07-29 — routing réaligné sur kct route)
...
⑥ call_agent_routing   → ROUTING_DONE
     ① kct route --strategy negotiated (CLI kicad-tools) — PRIMAIRE, circuits simples
        ...
        Flags : --auto-layers (escalade 2→4 couches) --min-completion 1.0 (cible 100%)
                --auto-fix (DRC pendant routage) --seed 42 (déterministe) --timeout 600
...
#### Pipeline routing — 5 niveaux, kct route PRIMAIRE (mis à jour 2026-07-29)
...
kct route input.kicad_pcb -o output.kicad_pcb \
  ...
  --timeout 600             # plafond, rend la main dès que 100% atteint
```
...
# Niveau 1 : kct route negotiated (PRIMAIRE, circuits simples ≤30 nets/comps, timeout 600s)
if is_simple:
    → kct_route.route_kct(timeout_s=600, vcc_as_traces=True)
...
# Niveau 4 : kct route negotiated SANS limite nets (réutilise kt_partial si dispo)
    ...
    → kct_route.route_kct(timeout_s=600)  # même algo que niveau 1, sans contrainte is_simple
