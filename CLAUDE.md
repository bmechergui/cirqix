...
└── kicad/          ← FastAPI Python headless KiCad
    ├── routers/kicad_gen.py      ← /circuit-synth/generate + /validate-symbols — génère .kicad_sch (via circuit_synth pip) + .kicad_pcb (Python S-expr)
...
FastAPI → kicad_gen.py handler :
           _generate_with_cs_lib() → circuit_synth pip (installé dans Docker) → .kicad_sch
           _generate_pcb_sexpr()   → Python S-expr pur → .kicad_pcb (toujours)
...
- Génération dual-mode :
  - Docker actif → `kicad_gen.py` appelle `_generate_with_cs_lib()` (circuit_synth pip local, installé via `pip install ./circuit_synth`) → `.kicad_sch`, puis `_generate_pcb_sexpr()` → `.kicad_pcb`
  - Docker absent → fallback inline S-expression TypeScript (`schematic-engine.ts` — `generateSchematic()` + `generatePCB()`)
