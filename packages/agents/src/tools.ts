...
export async function executeToolStub(
  ...
  switch (toolName) {
    ...
    case 'call_agent_routing': {
      ...
      try {
        // Strip TS-generated tracks before Freerouting: they point to
        // pre-placement positions and cause "unconnected end" DRC warnings.
        // Keep the original PCB so the skipped fallback still has traces.
        const originalPcbContent = base.kicad_pcb_content;
        const cleanPcbForFreerouting = stripTrackSegments(originalPcbContent);

        const service = await runRealRouting({
          kicadPcbContent: cleanPcbForFreerouting,
          layers: decidedLayers,
        });
        ...
        if (service.skipped) {
          return {
            ...
            kicad_pcb_content: originalPcbContent,
            engine: 'fallback-ts',
...
function stripTrackSegments(pcbContent: string): string {
  return pcbContent
    .split('\n')
    .filter(line => !line.trimStart().startsWith('(segment '))
    .join('\n');
}
